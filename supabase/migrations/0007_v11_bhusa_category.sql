-- Kisan Sahyog — 0007 (v1.1) Bhusa/Parali residue category
--
-- Reuses the existing `listings` table; the residue-specific fields live in the
-- `details` JSONB. Two changes: widen the category CHECK to admit 'bhusa', and
-- teach create_listing to validate the bhusa required fields. Asset-location
-- (coords from the residue's own pincode) is already handled generically in 0006.

-- Widen the category check (the v1 constraint was the inline auto-named one).
alter table public.listings drop constraint if exists listings_category_check;
alter table public.listings
  add constraint listings_category_check
  check (category in ('land', 'equipment', 'labor', 'bhusa'));
...
begin
  ...
  if p_category not in ('land', 'equipment', 'labor', 'bhusa') then
  ...
  if p_category = 'bhusa' then
    if coalesce(p_details->>'residue_type', '') = '' then
      raise exception 'residue_type_required';
    end if;
    if coalesce(trim(p_details->>'quantity'), '') = '' then
      raise exception 'quantity_required';
    end if;
    if coalesce(p_details->>'pickup_arrangement', '') = '' then
      raise exception 'pickup_required';
    end if;
    if coalesce(p_details->>'buyer_type_preference', '') = '' then
      raise exception 'buyer_type_required';
    end if;
    if coalesce(trim(p_details->>'asking_price'), '') = '' then
      raise exception 'asking_price_required';
    end if;
  end if;
