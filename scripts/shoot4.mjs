...
mkdirSync('/tmp/ks_shots', { recursive: true }); mkdirSync('docs/review', { recursive: true })
...
async function shoot(path, name, vp, tileW, tileH, keepFull) {
  ...
  await page.goto(BASE + path, { waitUntil: 'networkidle', timeout: 60000 }); await page.waitForTimeout(2200)
...
for (const [p,n] of [['/mausam','mausam'],['/fasal-salah','fasal'],['/msp','msp'],['/msp/gehun','gehun'],['/sawaal','sawaal']]) {
